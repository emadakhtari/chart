...
@section('content')
    <div class="page-header">
        <div>
            <h4><i class='fa fa-bar-chart ml-2'></i> لیست دستبندی نمودار ها </h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{route('home')}}">داشبورد</a></li>
                    <li class="breadcrumb-item active" aria-current="page">لیست دستبندی نمودار ها</li>
                </ol>
            </nav>
        </div>
        @can('Create ChartCategories')
            <div class="btn-group" role="group">
                <a href="{{ URL::to('chartCategories/create') }}" class="btn btn-success text-white">ایجاد دستبندی
                    نمودار</a>
            </div>
        @endcan
    ...
    <div class="card">
        <div class="card-body">
            <table id="example1" class="table table-striped table-bordered">
                ...
                @foreach ($chartCategories as $chartCategorie)
                    <tr>
                        <td>
                            {{$chartCategorie->id}}
                        </td>
                        ...
                        <td class="d-block">
                            @can('Edit ChartCategories')
                                <a href="{{ URL::to('chartCategories/'.$chartCategorie->id.'/edit') }}"
                                   class="btn btn-info text-white d-inline-block" style="margin-right: 3px;"><i
                                        class="icon ti-marker-alt"></i></a>
                            @endcan

                            @can('Delete ChartCategories')
                                {!! Form::open(['method' => 'DELETE', 'class'=>'d-inline-block', 'onsubmit' => 'return ConfirmDelete()' , 'route' => ['chartCategories.destroy', $chartCategorie->id] ]) !!}
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        @if (Route::currentRouteName() == 'chartCategories.index')
        ...
        $(".chartCategoriesList").addClass("active");
        @endif
    </script>
@stop
